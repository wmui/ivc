<template>
  ...
  @touchend="touchEnd"
  >
    <div class="ivc-swipe-items-container" ref="container">
      <slot></slot>
    </div>
    <div :class="['ivc-swipe-indicator-container', {'ivc-swipe-indicator-container-background': showTitle}]" v-if="showIndicators">
      <div v-if="showTitle" class="ivc-swipe-item-title">{{title}}</div>
      <ul :class="['ivc-swipe-indicators', {'ivc-swipe-indicators-right':
         indicatorsPosition === 'right'}]" v-if="indicatorsType === 'dot'">
        <li v-for="(page, pageIndex) in pages" :class="['ivc-swipe-indicator', {'current':pageIndex === index}]" :key="pageIndex"></li>
      </ul>
      <div v-if="indicatorsType === 'number'" class="ivc-swipe-number-indicator">{{index+1}} / {{pages.length}}</div>
    </div>
  </div>
</template>
...
.ivc-swipe-items-container {
  position: relative;
  overflow: hidden;
  height: 100%;
}

.ivc-swipe-items-container > .ivc-swipe-item {
  position: absolute;
...
.ivc-swipe-items-container > .ivc-swipe-item.current {
  display: block;
  transform: none;
}

.ivc-swipe-indicator-container {
  position: absolute;
  bottom: 0;
  width: 100%;
  height: 70px;
}
.ivc-swipe-indicator-container-background {
  background: rgba(0,0,0,.2)
}

.ivc-swipe-item-title {
  line-height: 70px;
  font-size: 30px;
  text-indent: 1em;
}

.ivc-swipe-indicators {
  position: absolute;
  bottom: 18px;
  left: 50%;
...
.ivc-swipe-indicator:first-child {
  margin-left: 0;
}
.ivc-swipe-indicator:last-child {
  margin-right: 0;
}
.ivc-swipe-indicator.current {
  opacity: 1;
}

.ivc-swipe-indicators-right {
  position: absolute;
  left: auto;
  right: 20px;
...
.ivc-swipe-vertical-indicators {
  position: absolute;
  right: 50px;
  top: 50%;
  transform: translateY(-50%);
}
.ivc-swipe-vertical-indicator {
  display: block;
  width: 5px;
  height: 5px;
...
.ivc-swipe-vertical-indicator::after,
.ivc-swipe-vertical-indicator::before {
  content: '';
  display: inline-block;
  width: 31px;
  height: 31px;
...
.ivc-swipe-vertical-indicator:hover,
.ivc-swipe-vertical-indicator.current {
  border-color: #fff;
  background-color: #258fb8;
}
.ivc-swipe-vertical-indicator.current::before,
.ivc-swipe-vertical-indicator.current::after {
  border-color: #258fb8;
