<template>
  ...
  >
    <div class="ivc-scroller-container" ref="scrollTarget">
      <!-- 上拉刷新 -->
      <div class="ivc-scroller-refresh" v-if="onRefresh" :class="{'ivc-scroller-refresh-no': isNoRefresh}">
        <!-- 无更多数据 -->
        ...
        <template v-else>
          <!-- refresh提示 -->
          <div class="ivc-scroller-refresh-pull" v-show="pullToRefreshState !== 2" :class="{'ivc-scroller-refresh-holding': pullToRefreshState === 1}">
              <i class="ivc-icon-refresh"></i>
              <p class="ivc-scroller-text">{{refreshText}}</p>
          </div>
          <!-- loading中 -->
          <div class="ivc-scroller-refresh-loading" v-show="pullToRefreshState === 2">
            <div class="ivc-scroller-spin">
             <i v-for="(item, index) in 12" :key="index"></i>
            </div>
             <p class="ivc-scroller-text">{{loadingText}}</p>
          </div>
        </template>
      </div>

      <div class="ivc-scroller-slot" ref="slotTarget">
        <slot></slot>
      </div>

      <!-- 无限加载 -->
      <div class="ivc-scroller-infinite" v-if="onLoad && contentOverflow" :class="{'ivc-scroller-infinite-no': isNoLoad}">
        ...
        <template v-else-if="isShowLoadingState">
          ...
          <div class="ivc-scroller-infinite-loading" v-show="infiniteLoadingState === 1">
            <div class="ivc-scroller-spin">
             <i v-for="(item, index) in 12" :key="index"></i>
            </div>
            <p class="ivc-scroller-text">{{loadingText}}</p>
...
export default {
  ...
  methods: {
    setInitData () {
      ...
      if (this.onRefresh) {
        this.tipHeight = this.$el.querySelector('.ivc-scroller-refresh').offsetHeight
      } else if (this.onLoad) {
        this.tipHeight = this.$el.querySelector('.ivc-scroller-infinite').offsetHeight
      }
      // 监视slot变化，更新scroller
...
.ivc-scroller {
  ...
  @for $i from 1 through 12 {
    &-spin > i:nth-child(#{$i}) {
      ...
      animation: ivc-spin 1.2s (-0.1*$i) * 1s infinite ease-in-out;
    }
  }
  &-spin > i {
